...
Создаёт графический слайдер (ползунок) для числового вопроса типа **Numeric**.

#### 💡 Особенности работы функции
* **Формат аргументов:** Параметры передаются в виде единого объекта.
* **Генерация разметки:** Слайдер размещается в динамически создаваемой таблице, которая вставляется в самое начало блока `.question_body` (методом `prepend`, перед `.inner_table`).
* **Классы и идентификаторы:** Созданной таблице присваивается класс `ui-tbl-slider` и `id="tbl_slider_<имя вопроса>_div"`. Сам ползунок доступен по селектору `div#slider_<имя вопроса>_div`.
* **Связка с оригинальным полем:** Исходное текстовое поле ввода (с `id`, равным имени вопроса) скрывается. Значение ползунка автоматически транслируется в него при перемещении — именно оно отправляется на сервер как ответ.
* **Логика фиксации ответа:**
  * Если у вопроса уже есть сохранённое значение (например, при возврате на страницу), ползунок сразу встаёт на эту позицию, и ответ считается данным.
  * Если значения изначально нет, ползунок визуально встаёт на позицию `value`, но оригинальное поле остаётся пустым (ответ не засчитывается), пока респондент вручную не сдвинет ползунок.
* **Зависимости:** Для работы скрипта необходимы библиотеки `jQuery` + `jQuery UI` (компонент `slider`), а также подключённый CSS-файл `slider-ui.css`, содержащий стили для создаваемой таблицы и элементов управления.

#### 📋 Свойства объекта-аргумента `opts`

| Свойство | Тип | Обязательный | По умолчанию | Описание |
| :--- | :---: | :---: | :---: | :--- |
| **ques** | `string` | **Да** | — | Имя вопроса. |
| **labelBefore** | `string` | Нет | `""` | Текст метки перед слайдером (подпись левого конца шкалы). |
| **labelAfter** | `string` | Нет | `""` | Текст метки после слайдера (подпись правого конца шкалы). |
| **handleVisible** | `boolean` | Нет | `true` | Видимость выбранного текущего числового значения на самом ползунке (`true` — отображать, `false` — скрыть). |
| **colorAnswered** | `string` | Нет | `"#008080"` | Цвет заливки трека ползунка, когда ответ уже предоставлен. |
| **colorNotAnswered** | `string` | Нет | `"#ff8040"` | Цвет слайдера при отсутствии ответа. <br>⚠️ *В текущей версии кода данный блок подсветки «без ответа» отключён.* |
| **sliderOpts** | `object` | Нет | `{}` | Нативный объект конфигурации ползунка [jQuery UI Slider](http://jqueryui.com). |

##### ⚙️ Параметры `sliderOpts` по умолчанию
Внутри функции дефолтные значения некоторых параметров jQuery UI Slider переопределены следующим образом:
```js
sliderOpts = {
  min: 0,        // Минимальное значение шкалы
  max: 100,      // Максимальное значение шкалы
  step: 1,       // Шаг изменения значений
  value: min,    // Текущее положение (по умолчанию совпадает с min)
  range: "min"   // Заливка трека цветом от начала шкалы до ползунка
};
```

#### Примеры использования

##### 1. Основной (все параметры по умолчанию, шкала 0–100, шаг 1)
```html
<link rel="stylesheet" type="text/css" href="https://marsurvey.ru">
<script>
\$(function() {
...
##### 2. Кастомная шкала 0–250 с подписями на концах
```html
<link rel="stylesheet" type="text/css" href="https://marsurvey.ru">
<script>
\$(function() {
...
##### 3. Полный набор параметров (анимация, цвета и кастомный шаг)
```html
<link rel="stylesheet" type="text/css" href="https://marsurvey.ru">
<script>
\$(function() {
  var q = {
    ...
    sliderOpts: {
      ...
      step: 5,
      value: 50,
      animate: true // Нативная опция jQuery UI для плавной анимации
    }
  };
